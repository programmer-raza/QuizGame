...
public class SubViewModel extends AndroidViewModel {
    ...
    public SubViewModel(@NonNull Application application) {
        ...
        highScore.setValue(prefs.getInt("HighScore_Subtraction", 0));
    }
    ...
    private void updateHighScoreIfNeeded() {
        ...
        int savedHigh = prefs.getInt("HighScore_Subtraction", 0);
        if (currentScore > savedHigh) {
            prefs.edit().putInt("HighScore_Subtraction", currentScore).apply();
